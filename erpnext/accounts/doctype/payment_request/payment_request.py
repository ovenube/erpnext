...
class PaymentRequest(Document):
	...
	def make_invoice(self):
		...
		if (hasattr(ref_doc, "order_type") and getattr(ref_doc, "order_type") == "Shopping Cart"):
			...
			si = make_sales_invoice(self.reference_name, ignore_permissions=True, is_autogenerated=True)
...
def make_payment_request(**args):
	...
	if args.order_type == "Shopping Cart":
		frappe.db.commit()
		if args.get('payment_method') == "culqi":
			response = pr.as_dict()
			response["culqi_info"] = pr.get_payment_settings()
			return response
		else:
