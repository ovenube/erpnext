		...
		for gle in gl_entries_data:
			if self.is_receivable_or_payable(gle, dr_or_cr, future_vouchers):
				...
				if abs(outstanding_amount) > 0.1/10**currency_precision:
					...
					# get original currency, amount and conversion rate					
					original_currency = voucher_details.get(gle.voucher_no, {}).get("currency", "")
					grand_total = voucher_details.get(gle.voucher_no, {}).get("grand_total", "")
					conversion_rate = voucher_details.get(gle.voucher_no, {}).get("conversion_rate", "")

					row += [original_currency, grand_total, conversion_rate]
	...
	def get_gl_entries(self, party_type, date=None, for_future=False):
		...
		self.gl_entries = frappe.db.sql("""
			...
			from
				`tabGL Entry`
			where
				docstatus < 2 and party_type=%s and voucher_type != "Journal Entry" and (party is not null and party != '') {1}
...
def get_voucher_details(party_type, voucher_nos, dn_details):
	...
	if party_type == "Customer":
		for si in frappe.db.sql("""select name, due_date, po_no, currency, grand_total, conversion_rate
	...
	if party_type == "Supplier":
		for pi in frappe.db.sql("""select name, due_date, bill_no, bill_date, currency, grand_total, conversion_rate
